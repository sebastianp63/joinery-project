<template>
  ...
  <span v-show="incorrectLastName || incorrectFirtName " class="uk-label uk-label-danger uk-text-large">Invalid user firstname or lastname </span>
  ...
  methods: {
    getFirstName() {
      ...
      if (this.firstName.match(letters)) {
        this.$emit("inputFirstName", this.firstName.charAt(0).toUpperCase() + this.firstName.slice(1));
    ...
    getLastName() {
      ...
      if (this.lastName.match(letters)) {
        this.$emit("inputLastName", this.lastName.charAt(0).toUpperCase() + this.lastName.slice(1));
